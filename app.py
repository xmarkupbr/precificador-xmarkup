...
@app.route('/api/precificacao/<int:prec_id>/add_derived_product', methods=['POST'])
@login_required
def add_derived_product(prec_id):
    data = request.get_json()
    if not data:
        return jsonify({'status': 'error', 'message': 'Nenhum dado recebido.'}), 400

    original_nfe_serie = data.get('original_nfe_serie')
    original_product_code = data.get('original_product_code')
    novo_codigo_derivado = data.get('novo_codigo_derivado')
    novo_nome_derivado = data.get('novo_nome_derivado')
    nova_qtd_derivado = data.get('nova_qtd_derivado')

    if not all([original_nfe_serie, original_product_code, novo_codigo_derivado, novo_nome_derivado, nova_qtd_derivado]):
        return jsonify({'status': 'error', 'message': 'Dados incompletos para criar produto derivado.'}), 400
    ...
    if not prec_record:
        ...
        if getattr(current_user, 'is_admin', False):
            prec_record = db.execute(
                "SELECT dados_json, parametros_json FROM precificacoes WHERE id = ?",
                (prec_id,)
            ).fetchone()
    ...
    parametros_json = json.loads(prec_record['parametros_json']) if prec_record['parametros_json'] else {}

    original_product = None
    original_product_index = -1
    for i, p in enumerate(produtos_json):
        if p.get("Série NF-e") == original_nfe_serie and p.get("Código") == original_product_code:
            original_product = p
            original_product_index = i
            break

    if not original_product:
        return jsonify({'status': 'error', 'message': 'Produto original não encontrado.'}), 404

    # Validação da quantidade
    if nova_qtd_derivado <= 0 or nova_qtd_derivado > original_product['Qtd']:
        return jsonify({'status': 'error', 'message': 'Quantidade derivada inválida ou maior que a quantidade disponível do produto original.'}), 400
    
    # Lógica de criação do produto derivado
    # Copia a maioria dos dados do produto original
    novo_produto = original_product.copy()
    novo_produto['Código'] = novo_codigo_derivado
    novo_produto['Nome'] = novo_nome_derivado
    novo_produto['Qtd'] = nova_qtd_derivado
    
    # Recalcula valor_total e Custo Unitário do derivado com base na nova Qtd
    # Reutiliza o Custo Unitário original (já calculado, que inclui impostos/fretes, etc.)
    # Se o custo unitário original já é "final", não é necessário refazer toda a proporção complexa aqui.
    # Apenas recalcula o valor_total para o derivado.
    original_unit_cost = original_product['Custo Unitário (R$)']
    novo_produto['valor_total'] = original_unit_cost * nova_qtd_derivado 
    novo_produto['Custo Unitário (R$)'] = original_unit_cost # O custo unitário é o mesmo do original
    novo_produto['Série NF-e'] = f"DERIVADO_{original_nfe_serie}" # Marca como derivado

    # Atualiza a quantidade do produto original
    original_product['Qtd'] -= nova_qtd_derivado
    original_product['valor_total'] = original_product['Custo Unitário (R$)'] * original_product['Qtd']

    # Adiciona o novo produto derivado à lista
    produtos_json.append(novo_produto)

    # Re-executa o cálculo de preços para todos os produtos (original e derivado)
    # Isso garante que as colunas de "Preço Venda Site", "Mercado Livre", "Shopee"
    # sejam calculadas corretamente para o novo produto e revalidadas para o original.
    recalculated_products = calculate_product_prices(produtos_json, 
    ...
    try:
        ...
        db.execute(
            ...
            (json.dumps(recalculated_products), prec_id)
        )
        db.commit()
        return jsonify({'status': 'success', 'message': 'Produto derivado criado e precificação atualizada com sucesso!'})
